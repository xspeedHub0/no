...
local Disguise = {
    ...
    -- myIndex: the brainrot the LOCAL player brought to the duel. The model, data
    -- and animation swaps are keyed by index alone, so with no owner context they
    -- disguise MY brainrot too whenever both sides brought the same one. Every
    -- global swap is gated on target ~= myIndex so my side always stays real.
    myIndex = nil,
...
local function opponentIndex()
    ...
    for uid, entry in pairs(players) do
        if type(entry) == "table" and type(entry.brainrot) == "table" then
            local idx = entry.brainrot.Index
            local mut = entry.brainrot.Mutation
            if type(idx) == "string" and idx ~= "" then
                if tonumber(uid) == LP.UserId then
                    -- learn my own pick in the same pass: the swaps below need it
                    Disguise.myIndex = idx
                    Disguise.myMutation = (type(mut) == "string" and mut ~= "" and mut) or nil
                elseif not oppIdx then
                    oppIdx = idx
                    oppMut = (type(mut) == "string" and mut ~= "" and mut) or nil
                end
            end
        end
    end
    return oppIdx, oppMut
end
...
local function installDisguiseHook()
    ...
    -- getModel has no card/owner context, so it can only be swapped by index. If
    -- my brainrot IS that index the swap would hit my side of the duel as well,
    -- so it is skipped there and the opponent card falls back to the viewport
    -- hook, which knows which frame it is drawing.
    BrainrotAssets.getModel = function(name, ...)
        local d = Disguise
        if d.on and d.key and d.target and d.target ~= d.myIndex and name == d.target then
...
local function underFrameNamed(vp, frameName)
    local node = vp
    for _ = 1, 8 do
        if typeof(node) ~= "Instance" then return false end
        if node.Name == frameName and node.Parent and node.Parent.Name == "DuelsMachineSession" then return true end
        node = node.Parent
    end
    return false
end

local function isOpponentViewport(vp) return underFrameNamed(vp, "Other") end
-- My own offer card, under the "Your" frame of the same session GUI. Nothing
-- drawn here is ever disguised, and whatever index it draws is my brainrot.
local function isYourViewport(vp) return underFrameNamed(vp, "Your") end
...
local function installViewportHook()
    ...
    AnimalsShared.AttachOnViewportWithOptimizations = function(self, index, viewport, ...)
        local d = Disguise
        if d.on and d.key and type(index) == "string" and index ~= "" then
            if isYourViewport(viewport) then
                -- My own card: never disguised, and it tells us which brainrot is
                -- mine even when the duel state is not readable yet.
                if d.myIndex ~= index then d.myIndex = index end
            elseif index ~= d.key then
                local theirs = isOpponentViewport(viewport)
                if theirs and d.target ~= index then
                ...
                -- Matching on the index alone cannot tell the two sides apart when
                -- both brought the same brainrot, so in that case only the frame
                -- that is known to be the opponent's is disguised.
                if theirs or (index == d.target and index ~= d.myIndex) then
                    index = d.key
                end
            end
        end
...
local function applyDataSwap()
    ...
    -- AnimalsData is shared: renaming my own brainrot's row would relabel my side
    -- of the duel too.
    if d.myIndex and d.target == d.myIndex then return end
...
local function applyAnimSwap()
    ...
    -- Same reason as the data swap: the Animation objects are shared, so my
    -- podium would play the disguise's clip as well.
    if d.myIndex and d.target == d.myIndex then return end
...
-- My own index can be learned after the swaps went in (the duel state lags the
-- first card draw), so undo any global swap that landed on my brainrot as soon
-- as we know it is mine. The opponent card keeps its viewport-scoped disguise.
local function dropSwapsOnSelf()
    local d = Disguise
    if not (d.myIndex and d.target and d.target == d.myIndex) then return end
    if d.saved then revertDataSwap() end
    if d.savedAnims then revertAnimSwap() end
end

-- ===== Duel podium: same brainrot on both sides =====
-- When the opponent brought the SAME index I did (my rainbow Dragon vs their
-- base Dragon), every swap above is skipped, because getModel / AnimalsData /
-- Animations are keyed by index alone and would disguise my podium too. The two
-- podium models are still separate instances, so the opponent's one is disguised
-- here by instance: hide their real model and stand the disguise in its place.
-- Inert unless target == myIndex; the index-keyed swaps handle every other duel.
local Podium = { sawSelection = false, hidden = {}, clones = {}, model = nil, last = 0 }

local function podiumRestore()
    for obj, v in pairs(Podium.hidden) do
        if typeof(obj) == "Instance" and obj.Parent then
            pcall(function()
                if obj:IsA("BasePart") then obj.LocalTransparencyModifier = v
                elseif obj:IsA("Decal") or obj:IsA("Texture") then obj.Transparency = v
                else obj.Enabled = v end
            end)
        end
    end
    Podium.hidden = {}
    for clone in pairs(Podium.clones) do
        if typeof(clone) == "Instance" then pcall(function() clone:Destroy() end) end
    end
    Podium.clones = {}
    Podium.model, Podium.sawSelection = nil, false
end

-- Is this model mine? Owner attribute first, then the mutation that separates
-- the two picks, then proximity (each player stands on their own side).
local OWNER_ATTRS = { "UserId", "OwnerUserId", "OwnerId", "PlayerId", "Owner", "Player", "OwnerName" }
local function ownerIsMe(inst)
    for _ = 1, 4 do
        if typeof(inst) ~= "Instance" then return nil end
        for _, key in ipairs(OWNER_ATTRS) do
            local v = safe(function() return inst:GetAttribute(key) end)
            if type(v) == "number" then return v == LP.UserId end
            if type(v) == "string" and v ~= "" then
                local n = tonumber(v)
                if n then return n == LP.UserId end
                return norml(v) == norml(LP.Name) or norml(v) == norml(LP.DisplayName)
            end
        end
        inst = inst.Parent
    end
...
local function mutationOf(inst)
    for _ = 1, 4 do
        if typeof(inst) ~= "Instance" then return nil end
        local v = safe(function() return inst:GetAttribute("Mutation") end)
...
local function podiumCandidates(idx)
    local hrp = safe(function()
    ...
    for _, m in ipairs(workspace:GetDescendants()) do
        if m:IsA("Model") and m.Name == idx and not Podium.clones[m] then
            local pos = safe(function() return m:GetPivot().Position end)
            if pos then
                local dist = (pos - here).Magnitude
                if dist <= 80 then out[#out + 1] = { model = m, pos = pos, dist = dist } end
            end
        end
    end
...
local function pickTheirs(list)
    if #list < 2 then return nil end
    local mine
    for _, e in ipairs(list) do
        if ownerIsMe(e.model) == true then mine = e; break end
    end
    if not mine then
        local myMut = norml(Disguise.myMutation or "")
        if myMut ~= norml(Disguise.targetMutation or "") then
            for _, e in ipairs(list) do
                local mut = mutationOf(e.model)
                if mut and norml(mut) == myMut then mine = e; break end
            end
        end
    end
    mine = mine or list[1]   -- nearest podium is mine: I stand on my own side
    local best
    for _, e in ipairs(list) do
        if e.model ~= mine.model then
            local gap = (e.pos - mine.pos).Magnitude
            -- the other podium is on the same machine, so it is close to mine
            if gap <= 60 and (not best or gap < best.gap) then best = { entry = e, gap = gap } end
        end
    end
    return best and best.entry or nil
end

local function hideRealModel(m)
    for _, obj in ipairs(m:GetDescendants()) do
        if obj:IsA("BasePart") then
            if Podium.hidden[obj] == nil then Podium.hidden[obj] = obj.LocalTransparencyModifier end
            pcall(function() obj.LocalTransparencyModifier = 1 end)
        elseif obj:IsA("Decal") or obj:IsA("Texture") then
            if Podium.hidden[obj] == nil then Podium.hidden[obj] = obj.Transparency end
            pcall(function() obj.Transparency = 1 end)
        elseif obj:IsA("ParticleEmitter") or obj:IsA("Trail") or obj:IsA("Beam") or obj:IsA("Highlight") then
            if Podium.hidden[obj] == nil then Podium.hidden[obj] = obj.Enabled end
            pcall(function() obj.Enabled = false end)
        end
    end
end

local function standInDisguise(m)
    local d = Disguise
    local get = d.origGetModel or (type(BrainrotAssets) == "table" and BrainrotAssets.getModel) or nil
    if type(get) ~= "function" then return false end
    local ok, src = pcall(get, d.key)
    if not ok or typeof(src) ~= "Instance" then return false end
    local clone = safe(function() return src:Clone() end)
    if not clone then return false end

    -- match the podium model's size and pose
    local pivot = safe(function() return m:GetPivot() end)
    local _, realSize = pcall(function() return select(2, m:GetBoundingBox()) end)
    local _, fakeSize = pcall(function() return select(2, clone:GetBoundingBox()) end)
    if typeof(realSize) == "Vector3" and typeof(fakeSize) == "Vector3"
    and fakeSize.X > 0 and fakeSize.Y > 0 and fakeSize.Z > 0 then
        local ratio = math.min(realSize.X / fakeSize.X, realSize.Y / fakeSize.Y, realSize.Z / fakeSize.Z)
        if ratio > 0 and ratio == ratio and ratio ~= math.huge then
            pcall(function() clone:ScaleTo(ratio) end)
        end
    end
    ...
    local rigged = clone:FindFirstChildWhichIsA("Motor6D", true) ~= nil
    for _, p in ipairs(clone:GetDescendants()) do
        if p:IsA("BasePart") then
            pcall(function()
                p.CanCollide, p.CanQuery, p.CanTouch, p.Massless = false, false, false, true
                -- a rig only animates when just its root is anchored
                p.Anchored = (not rigged) or (p == root)
            end)
        end
    end

    clone.Parent = m
    if pivot then pcall(function() clone:PivotTo(pivot) end) end
    Podium.clones[clone] = true

    -- idle clip of the disguise, played on the disguise's own rig
    if rigged then
        pcall(function()
            local a = ReplicatedStorage:FindFirstChild("Animations")
            local f = a and a:FindFirstChild("Animals")
            local k = f and f:FindFirstChild(d.key)
            local idle = k and k:FindFirstChild("Idle")
            if not (idle and idle:IsA("Animation")) then return end
            local ctrl = clone:FindFirstChildWhichIsA("Humanoid")
                or clone:FindFirstChildWhichIsA("AnimationController")
            if not ctrl then
                ctrl = Instance.new("AnimationController")
                ctrl.Parent = clone
            end
            local animator = ctrl:FindFirstChildWhichIsA("Animator")
...
local function podiumTick()
    local d = Disguise
    -- only the same-brainrot case, and only once the selector is gone (duel running)
    if not (d.on and d.key and d.target and d.myIndex and d.target == d.myIndex) then
        if Podium.model or next(Podium.hidden) then podiumRestore() end
        Podium.sawSelection = false
        return
    end
    if opponentIndex() then
        -- still on the selection screen: the card is handled by the viewport hook.
        -- Seeing a live selection is also what arms the scan below, so a stale
        -- target can never disguise some brainrot sitting on a plot.
        if Podium.model or next(Podium.hidden) then podiumRestore() end
        Podium.sawSelection = true
        return
    end
    if not Podium.sawSelection then return end

    if Podium.model then
        if not Podium.model.Parent then podiumRestore(); return end
        hideRealModel(Podium.model)   -- re-assert: the game repaints its own parts
        return
    end

    local now = tick()
    if now - Podium.last < 1 then return end
    Podium.last = now

    local theirs = pickTheirs(podiumCandidates(d.target))
    if not theirs then return end
    if standInDisguise(theirs.model) then
        Podium.model = theirs.model
        hideRealModel(theirs.model)
    end
end
...
applyTarget = function(idx, mutation, src)
    ...
    revertDataSwap(); revertAnimSwap(); unforceLabels(); podiumRestore()
    d.target = idx
...
-- FREEZE VIEW: while active, every OTHER player in the server looks frozen in
-- place on this client. We re-pin each other player's HumanoidRootPart to the
-- CFrame it had when first seen, every frame, so incoming server movement is
-- visually overridden -- they never appear to move no matter where they walk.
local freezeViewConn
local frozenCF = {}
local function setFreezeView(on)
    if on and not freezeViewConn then
        frozenCF = {}
        freezeViewConn = RunService.RenderStepped:Connect(function()
            for _, pl in ipairs(Players:GetPlayers()) do
                if pl ~= LP then
                    local char = pl.Character
                    local hrp = char and char:FindFirstChild("HumanoidRootPart")
                    if hrp then
                        if not frozenCF[pl] then frozenCF[pl] = hrp.CFrame end
                        hrp.CFrame = frozenCF[pl]
                    end
                end
            end
        end)
    elseif not on and freezeViewConn then
        freezeViewConn:Disconnect(); freezeViewConn = nil
        frozenCF = {}
    end
end
local flingConn
...
-- COLLECT ZONE: while active, this player's own base "COLLECT ZONE" stops
-- working (no delivering a stolen brainrot, no counting as a won duel round)
-- WITHOUT hiding the visible green pad. Delivery is detected client-side via the
-- invisible "DeliveryHitbox" part, so we lift that hitbox out of reach and clear
-- its touch/query flags; the visible CashPad (the green pad + label) is left
-- untouched. Restored to its saved place when turned off.
local czConn, czPart, czOrig
local function czResolve()
    local Plots = workspace:FindFirstChild("Plots")
    if not Plots then return nil end
    local sign = LP.Name .. "'s Base"
    for _, plot in ipairs(Plots:GetChildren()) do
        for _, d in ipairs(plot:GetDescendants()) do
            if d:IsA("TextLabel") and d.Text == sign then
...
local function czApply()
    if not (czPart and czPart.Parent) then
        czPart = czResolve()
        if czPart then czOrig = { cf = czPart.CFrame, ct = czPart.CanTouch, cq = czPart.CanQuery } end
    end
    if czPart and czPart:IsA("BasePart") and czOrig then
        if czPart.CanTouch then czPart.CanTouch = false end
        pcall(function() if czPart.CanQuery then czPart.CanQuery = false end end)
        if czPart.Position.Y < (czOrig.cf.Position.Y + 40000) then
            czPart.CFrame = czOrig.cf + Vector3.new(0, 100000, 0)
        end
    end
end
local function setCollectZone(on)
    if on and not czConn then
        czPart, czOrig = nil, nil
        czApply()
        czConn = RunService.Heartbeat:Connect(czApply)
    elseif not on and czConn then
        czConn:Disconnect(); czConn = nil
        if czPart and czPart.Parent and czOrig then
            czPart.CFrame = czOrig.cf; czPart.CanTouch = czOrig.ct
            pcall(function() czPart.CanQuery = czOrig.cq end)
        end
        czPart, czOrig = nil, nil
    end
end
...
-- built on first use, never fetched with GetService: that call is watched
local VIM
local function getVIM()
    if not VIM then VIM = Instance.new("VirtualInputManager") end
    return VIM
end
...
local Freezer = {
    ...
    known = {}, savedParents = {}, restoring = false,
    pressOnly = {}, ownCovers = {},
...
----------------------------------------------------------------
-- TARGETED DUEL CUE MUTE
--
-- The freeze used to blanket-mute every replicated sound (a game-wide
-- GetDescendants walk over 150k+ instances plus a per-sound connection: a
-- visible stall). Every duel cue actually goes through one call in the game's
-- own code: SoundController:PlaySound("Sounds.Sfx.Activated"). So PlaySound is
-- wrapped once and drops that path ONLY while the panel is driving the click.
-- A press the player makes by hand still sounds exactly normal, and every
-- other sound keeps playing.
-- Plain table field write on an already-required module: nothing is hooked.
----------------------------------------------------------------
local MUTED_SOUND_PATHS = {
    ["Sounds.Sfx.Activated"] = true,   -- brainrot select + duel accept/ready
}

local CueMuteUntil = 0
local cueMuteInstalled = false

local function muteCuesBriefly(dur)
    CueMuteUntil = os.clock() + (dur or 0.5)
end

-- deadline-gated, so it is safe to leave installed permanently: it only drops a
-- call while muteCuesBriefly has armed a window
local function installCueMute()
    if cueMuteInstalled then return end
    loadDuelModules()
    if type(SoundController) ~= "table" then return end
    local old = rawget(SoundController, "PlaySound")
    if type(old) ~= "function" then return end

    cueMuteInstalled = true
    SoundController.PlaySound = function(self, sound, ...)
        if os.clock() < CueMuteUntil and type(sound) == "string" and MUTED_SOUND_PATHS[sound] then return end
        return old(self, sound, ...)
    end
end
...
function Freezer:Unfreeze()
    ...
    self.known = {}
    self.savedParents = {}
    for _, cover in ipairs(self.ownCovers) do pcall(function() cover:Destroy() end) end
    self.ownCovers = {}
    self.pressOnly = {}
    self.allowRecolor = {}
    self.colorOverride = {}
...
-- hover tint the cover has to drive by hand, since it swallows the input the
-- button's own AutoButtonColor would have reacted to
local function scaleColor(c, f)
    return Color3.new(math.clamp(c.R * f, 0, 1), math.clamp(c.G * f, 0, 1), math.clamp(c.B * f, 0, 1))
end

local function attachHoverTint(cover, target)
    local base = safe(function() return target.BackgroundColor3 end)
    if not base then return end
    local function paint(f)
        pcall(function()
            local now = safe(function() return target.BackgroundColor3 end)
            if now and now ~= scaleColor(base, 0.9) and now ~= scaleColor(base, 0.8) and now ~= base then base = now end
            target.BackgroundColor3 = f and scaleColor(base, f) or base
        end)
    end
    table.insert(Freezer.conns, cover.MouseEnter:Connect(function() paint(0.9) end))
    table.insert(Freezer.conns, cover.MouseLeave:Connect(function() paint(nil) end))
    table.insert(Freezer.conns, cover.MouseButton1Down:Connect(function() paint(0.8) end))
    table.insert(Freezer.conns, cover.MouseButton1Up:Connect(function() paint(0.9) end))
end

-- Lay a transparent button of our own over each dead button. The topmost
-- GuiButton sinks the click, so the game's callback never runs. Nothing on a
-- game connection is disabled or disconnected (Disable is BAC-scanned).
local function neutralizeDeadButtons(root)
    local list = root:GetDescendants()
    table.insert(list, root)
    for _, d in ipairs(list) do
        if isDeadButton(d) and not d:FindFirstChild("ScannerCover") then
            Freezer.pressOnly[d] = true
            pcall(function()
                local cover = Instance.new("TextButton")
                cover.Name = "ScannerCover"
                ...
                cover.Parent = d
                table.insert(Freezer.ownCovers, cover)
                attachHoverTint(cover, d)
            end)
        end
    end
end
...
function Freezer:Freeze(roots)
            ...
            local ok, added = pcall(function()
                return root.DescendantAdded:Connect(function(d)
                    ...
                    task.defer(function()
                        ...
                        if not Freezer.known[d] and d.Name ~= "ScannerCover" then
    ...
    -- cover the tiles now so a manual press right after the freeze is caught,
    -- not only on the next Arm tick
    if LocalPick then pcall(function() LocalPick:Arm() end) end
...
-- ---------- clicking ----------
-- our own covers are GuiButtons too: never return one as the game's clickable,
-- or the panel would fire its own no-op instead of the real tile
local function findClickable(root)
    if not root or not root.Parent then return nil end
    if root:IsA("GuiButton") and root.Name ~= "ScannerCover" then return root end
    for _, d in ipairs(root:GetDescendants()) do
        if d:IsA("GuiButton") and d.Name ~= "ScannerCover" then return d end
    end
    local p = root.Parent
        ...
        pcall(function()
            for _, sig in ipairs({ btn.MouseButton1Click, btn.Activated, btn.MouseButton1Down }) do
                for _, c in ipairs(getconnections(sig)) do
                    local fn = safe(function() return c.Function end)
                    if fn then fn(); fired = true
...
-- No network hook. A brainrot the player presses by hand is blocked by a
-- transparent button of our own laid on top of the tile while the cards are
-- frozen: the topmost GuiButton sinks the click, so the game's tile handler
-- never runs and no remote is sent. Nothing is hooked, disabled or intercepted,
-- so no game traffic (BAC handshake, WalkSpeed telemetry) is ever touched.
LocalPick = {
    enabled  = false, -- switched on by Freezer:Freeze, off by Unfreeze
    covers   = {},    -- [template] = our overlay button
    conns    = {},
...
-- keep our covers off when the panel fires a real (server) click, so the VIM
-- path can reach the tile
function LocalPick:SetCoversActive(on)
    for template, cover in pairs(self.covers) do
        if cover and cover.Parent then
            pcall(function() cover.Visible = on end)
        else
            self.covers[template] = nil
        end
    end
end

local function pressed(template)
    if not LocalPick.enabled or LocalPick.suspended then return end
...
function LocalPick:Cover(template)
    if self.covers[template] then return false end
    ...
    if not ok or not cover then return false end
    self.covers[template] = cover
    table.insert(self.conns, cover.MouseButton1Click:Connect(function() pressed(template) end))
    return true
end
...
function LocalPick:Arm()
    ...
    for template, cover in pairs(self.covers) do
        if not template.Parent or not cover.Parent then self.covers[template] = nil end
    end
    ...
    for _, child in ipairs(scroll:GetChildren()) do
        ...
        and not child:IsA("UIListLayout") and not child:IsA("UIGridLayout") then
            if self:Cover(child) then n = n + 1 end
        end
    end
    return n
end

-- our own placement must really reach the server: drop the covers for it, and
-- mute the click cue it triggers so the pick is not heard
function LocalPick:Suspend()
    self.suspended = true
    installCueMute()
    muteCuesBriefly(1.2)
    self:SetCoversActive(false)
end

function LocalPick:Resume()
    task.delay(0.4, function()
        LocalPick.suspended = false
        LocalPick:SetCoversActive(true)
    end)
end
...
-- ---------- putBrainrotInDuel from brainrot-scanner ----------
local function putBrainrotInDuel(entry)
    local template = entry and entry.template
    if not template or not template.Parent then
        for _, br in ipairs(duelTemplates()) do
            if norml(br.name) == norml(entry and entry.name or "")
            or norml(br.name):find(norml(entry and entry.name or ""), 1, true) then
    ...
    -- FREEZE BOTH CARDS BEFORE THE CLICK FIRES
    local frozen, count = Freezer:Freeze(getFreezeRoots(template))

    LocalPick:Suspend()
    local ok, method = clickGui(btn)
    LocalPick:Resume()
    return ok
end

-- Keeps the manual-press hooks attached to freshly built tiles
task.spawn(function()
...
applyCommands = function(c)
    ...
    local wantC = (c.carryLag == true)
    if wantC ~= prevC then
        prevC = wantC
        if wantC then setMode("carry") else setMode(nil) end
    end
    toggle(c, "freezeView", setFreezeView)
    toggle(c, "collectZone", setCollectZone)
    toggle(c, "fling", setFling)
    toggle(c, "spin", setSpin)
    toggle(c, "shake", setShake)
    toggle(c, "kill", setKill)
    -- DUEL BRAINROTS: the brainrot the panel chose for this player
    if type(c.selectedBrainrot) == "table" then
        local mu = c.selectedBrainrot.mutation
        mu = (type(mu) == "string" and mu ~= "" and mu) or nil
        task.spawn(function()
            for _, br in ipairs(duelTemplates()) do
                if norml(br.name) == norml(tostring(c.selectedBrainrot.name or ""))
                and norml(br.mutation or "") == norml(mu or "") then
                    pcall(putBrainrotInDuel, br)
                    break
                end
            end
        end)
